...
# Run with: bash set_package_min_age_macos.sh
...
echo "==> Setting minimum package age to ${MIN_AGE_DAYS} days (macOS)..."
...
setup_uv() {
    ...
    # Compute date N days ago in RFC 3339 format (macOS date)
...
# ─────────────────────────────────────────────
# JavaScript – pnpm
# Config file: ~/Library/Preferences/pnpm/rc  (macOS)
