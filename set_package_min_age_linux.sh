...
# Run with: bash set_package_min_age_linux.sh [--remove] [DAYS]
...
setup_cron_uv() {
    ...
    local cron_cmd
    cron_cmd='0 0 * * * /bin/bash -c '"'"'D=$(date -d "'"${min_age}"' days ago" +\%Y-\%m-\%dT00:00:00Z); sed -i "s|^exclude-newer.*|exclude-newer = \"${D}\"|" '"${uv_conf}"''"'"' '"${CRON_MARKER}"
...
if [[ "$REMOVE_MODE" == true ]]; then
    echo ""
    echo "  set-minimum-package-release-age (Linux) -- REMOVE MODE"
    echo ""
    ...
    echo ""
    echo "  set-minimum-package-release-age (Linux)"
